...
class SSHTunnelManager:
    ...
    def _start_with_system_ssh(self) -> bool:
        """使用系统 SSH 命令建立隧道"""
        # 🔌 支持 SSH Config 别名（如 "ssh pc"）
        # 如果 ssh_user 为空，说明使用的是 SSH 别名
        if self.config.ssh_user:
            host_spec = f"{self.config.ssh_user}@{self.config.remote_host}"
        else:
            host_spec = self.config.remote_host  # 直接使用别名，如 "pc"
...
def create_tunnel_from_env() -> Optional[SSHTunnelManager]:
    ...
    环境变量:
        ...
        SSH_TUNNEL_USER: SSH 用户名（可选，如果用 SSH Config 别名可省略）
    ...
    示例:
        # 使用 SSH Config 别名（推荐，已配置好 ~/.ssh/config）
        SSH_TUNNEL_HOST=pc
        
        # 或完整配置
        SSH_TUNNEL_HOST=192.168.1.100
