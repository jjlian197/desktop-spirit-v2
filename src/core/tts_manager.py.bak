...
class TTSManager(QObject):
    ...
    async def _play_audio(self, audio_path: str):
        ...
        # Start lip sync timer (30fps = 33ms per frame)
        if self._amplitude_data:
            self._playback_timer.start(33)  # ~30fps
    ...
    def _on_playback_frame(self):
        ...
        if self._current_frame < len(self._amplitude_data):
            ...
            # Map amplitude to mouth opening (0.0 - 1.0)
            # Use a curve for more natural movement
            mouth_open = min(amplitude * 1.5, 1.0)  # Scale up slightly
