...
# Windows API for mouse tracking and screen info
...
class SpriteBrain:
    def __init__(self, ws_uri="ws://127.0.0.1:8765/sprite", http_port=8766):
        ...
        # 鼠标跟随配置 - 🚨 默认禁用，由 live2d_view 自己控制鼠标跟随
        self.mouse_config = {
            "enabled": False,  # 由前端 Live2DView 自己控制鼠标跟随
        ...
        self.idle_config = {
            ...
            "idle_timeout": 10,        # 进入空闲状态所需秒数（无操作），调低以便测试
        ...
        while self.running:
            try:
                ...
                async with websockets.connect(self.ws_uri) as ws:
                    ...
                    receive_task = asyncio.create_task(self._receive_loop())  # 接收消息
    ...
    async def speak(self, text: str, interactive: bool = True):
        """
        说话 - 🚨 使用互斥锁防止多个语音同时播放，避免气泡和语音内容不匹配
        Args:
            text: 要说的内容
        ...
        async with self._speak_lock:
            logger.info(f"🗣️ 开始播放语音: {text[:30]}...")
            
            # 发送语音命令（WebSocket 服务器会处理回正和气泡显示）
    ...
    async def _idle_loop(self):
        ...
        while self.running:
            ...
            # 每10秒输出一次空闲状态调试信息
            if not hasattr(self, '_last_idle_debug'):
                self._last_idle_debug = 0
            if int(idle_time) % 10 == 0 and int(idle_time) != self._last_idle_debug:
    ...
    async def _handle_touch(self, action: str, part: str):
        ...
        # 🚨 尝试触发动画（可选，失败不阻断流程）
        # 注意：如果模型没有对应的动作组，会静默失败
        try:
            motion_result = await self.trigger_motion(reaction["motion"])
            if not motion_result:
                logger.debug(f"Motion '{reaction['motion']}' not available in model")
        except Exception as e:
    ...
    def get_mouse_position(self):
        ...
        norm_y = -((y / self.screen_height * 2) - 1)  # 反转Y轴
    ...
    async def _start_http_server(self):
        ...
        app.router.add_post("/api/tts", self._handle_http_tts)
